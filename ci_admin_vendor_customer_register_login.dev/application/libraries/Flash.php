...
class Flash {
	...
	/**
	 * The session name used for userdata.
	 *
	 * @var string
	 * @access public
	 */
	 ...
	 * will be stored in userdata for the next one.
	 *
	...
	private function _add_message($message, $data = '', $type = 'default', $display_now = FALSE)
	{
		...
		if ($display_now === FALSE) {
			...
			$this->_ci->session->set_userdata($this->session_name, $this->_session_messages);
		}
		else {
	...
	{
		$session_messages = $this->_ci->session->userdata($this->session_name);
...
*/
	...
	public function display($type = '', $split = NULL)
	{
		...
		$this->_ci->session->unset_userdata($this->session_name);
		return $output;
	}
